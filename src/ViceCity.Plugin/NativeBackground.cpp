...
static D2D1_RECT_F glassRects[2]={{3963,852,4742,1070},{3963,1086,4742,1295}};
...
static std::filesystem::path Output(){wchar_t local[MAX_PATH];GetEnvironmentVariableW(L"LOCALAPPDATA",local,MAX_PATH);return std::filesystem::path(local)/L"ViceCityRainmeter";}
struct Paint {
    ...
    void Draw(){
        ...
        auto quiet=[](float x,float y){double a=(x-4352)/(779*.68),b=(y-720)/(1150*.7);return float(.25+.75*(1-std::exp(-(a*a+b*b)*1.7)));};
        for(int i=0;i<14;i++){
        ...
        if(frosted){
            D2D1_RECT_F rects[2];{std::lock_guard<std::mutex> lock(glassMutex);rects[0]=glassRects[0];rects[1]=glassRects[1];}
            ComPtr<ID2D1Factory> factory;rt->GetFactory(&factory);
            for(auto r:rects){
                if(r.right<=r.left||r.bottom<=r.top)continue;
                float radius=24*(r.right-r.left)/779;
                ...
                rt->DrawBitmap(frosted.Get(),r,1,D2D1_BITMAP_INTERPOLATION_MODE_LINEAR,D2D1::RectF(r.left-4+shift,r.top-3,r.right+4+shift,r.bottom+3));
                Fill(r,D2D1::ColorF(.06f,.022f,.10f,.46f));
                sheen->SetStartPoint(D2D1::Point2F(r.left,r.top));sheen->SetEndPoint(D2D1::Point2F(r.right,r.bottom));
                rt->FillRectangle(r,sheen.Get());rt->PopLayer();
...
static DWORD WINAPI Run(void*){
    ...
    HWND window=nullptr;
    try{
        WNDCLASSW cls{};cls.lpfnWndProc=Proc;cls.hInstance=GetModuleHandleW(nullptr);cls.lpszClassName=L"ViceCityNativeBackground";RegisterClassW(&cls);
        window=CreateWindowExW(WS_EX_TOOLWINDOW|WS_EX_NOACTIVATE|WS_EX_LAYERED,cls.lpszClassName,L"ViceCity native background",WS_CHILD|WS_VISIBLE,0,0,5120,1440,parentWindow,nullptr,cls.hInstance,nullptr);
        if(!window)throw HRESULT_FROM_WIN32(GetLastError());
        if(!SetLayeredWindowAttributes(window,0,255,LWA_ALPHA))throw HRESULT_FROM_WIN32(GetLastError());
        ...
        Paint paint(target.Get(),wic.Get());
        auto last=std::chrono::steady_clock::now();bool first=true;
        while(WaitForSingleObject(stopEvent,0)!=WAIT_OBJECT_0){
            MSG msg;while(PeekMessageW(&msg,nullptr,0,0,PM_REMOVE)){TranslateMessage(&msg);DispatchMessageW(&msg);}
            bool paused=Fullscreen();auto now=std::chrono::steady_clock::now();double dt=std::min(.1,std::chrono::duration<double>(now-last).count());last=now;
    ...
    }catch(HRESULT hr){std::ofstream log(Output()/L"native-renderer-error.txt");log<<"HRESULT "<<std::hex<<hr;}catch(...){std::ofstream log(Output()/L"native-renderer-error.txt");log<<"Native renderer failed";}
    if(window)DestroyWindow(window);UnregisterClassW(L"ViceCityNativeBackground",GetModuleHandleW(nullptr));CoUninitialize();return 0;
}
...
void SetGlass(float x,float cy,float my,float w,float ch,float mh){
    if(w<0||w>1200||ch<0||ch>1440||mh<0||mh>1440||x<2560||x+w>5120)return;
