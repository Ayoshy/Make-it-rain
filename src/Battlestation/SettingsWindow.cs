...
internal sealed class SettingsWindow : Window
{
    ...
    bool ready;
    internal SettingsWindow(Station state,PaletteHotkey shortcut,Func<string,bool,bool> visibility,Action organize,Action<Window> editApps)
    {
        ...
        Closed+=(_,_)=>Native.BackgroundAppearance(station.Settings.AnimateBackground?1:0,(float)station.Settings.GlassOpacity);
    ...
    void Save()
    {
        try
        {
            ...
            station.ApplySettings(new DesktopSettings(projectRoot.Text,city.Text,Number(latitude.Text),Number(longitude.Text),animation.IsChecked==true,1-transparency.Value/100),target);feedback.Text="Réglages enregistrés";
        }
