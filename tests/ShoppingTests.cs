...
try
{
    ...
    Equal<decimal?>(850,withModel.MaxPrice,"Le modèle affine le prix");
    ...
    Check(events.Any(item=>item.Label=="Prime Day"&&item.Shop=="Amazon"),"Le Prime Day est rattaché à Amazon");
    ...
    Equal("French Days d'automne",calendar.Next(new DateOnly(2026,9,17),30)?.Label??"","Le prochain rendez-vous national est trouvé depuis une date");
    Equal("French Days d'automne",calendar.Next(new DateOnly(2026,9,17),30,"Rue du Commerce")?.Label??"","Sans repère d'enseigne, le rendez-vous national s'applique");
    ...
    Equal("French Days d'automne",loaded.Next(new DateOnly(2026,9,17),30,"Rue du Commerce")?.Label??"","Un repère d'une autre enseigne ne s'impose pas");
    ...
    using(var store=new ShoppingStore(Path.Combine(temp,"engine.db")))
    {
        ...
        using var service=new ShoppingService(store,[cheap,dearer,rejected],settings,temp,new HttpClient(),engine,calendar,new ShoppingSpecParser());
        ...
        cheap.NextPrice=(watched.Product.Price??300m)-40m;
        ...
        using var second=new ShoppingService(store,[cheap,failing],settings,temp,new HttpClient(),engine,calendar,new ShoppingSpecParser());
    ...
    using(var quiet=new ShoppingService(quietStore,[new FakeSource("delta","Delta",[])],settings,temp,new HttpClient(),engine,calendar,new ShoppingSpecParser()))
    {
    ...
    using(var dead=new ShoppingService(deadStore,[new FakeSource("delta","Delta",[]){Failure="Delta refuse la lecture"}],settings,temp,new HttpClient(),engine,calendar,new ShoppingSpecParser()))
    {
    ...
    if(args.Contains("--live"))
    {
        ...
        using var radar=new ShoppingService(store,live,settings,temp,http,engine,calendar,new ShoppingSpecParser());
...
sealed class FakeSource(string id,string name,IReadOnlyList<Product> products):IPriceSource
{
    ...
    public Task<PricePoint?> GetPriceAsync(Product product,CancellationToken cancellation)
        =>Failure.Length>0?Task.FromException<PricePoint?>(new PriceSourceUnavailableException(Failure)):Task.FromResult<PricePoint?>(new(product.Id,DateTimeOffset.Now,NextPrice??product.Price,"EUR",true,id));
}
